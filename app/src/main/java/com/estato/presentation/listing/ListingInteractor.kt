...
class ListingInteractor @Inject constructor(
    private val loadRealEstatesWithRefreshUseCase: LoadRealEstatesWithRefreshUseCaseInterface,
    ...
    private fun loadRealEstates(): Flow<ListingResult> {
        return loadRealEstatesWithRefreshUseCase.execute()
    ...
    private fun refreshRealEstates(): Flow<ListingResult> {
        return flow {
            ...
            try {
                ...
                emit(ListingResult.Error("Network error: ${e.message}"))
            } catch (e: HttpException) {
                emit(ListingResult.Error("Server error: ${e.message}"))
            }
        }
    }
}
